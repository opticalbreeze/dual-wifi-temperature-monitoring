...
echo "=== 6. wlan1-setup サービス状態 ==="
sudo systemctl status wlan1-setup.service 2>&1 | head -20
echo ""

echo "=== 7. temperature-server サービス状態 ==="
sudo systemctl status temperature-server 2>&1 | head -20
echo ""

echo "=== 8. wlan1 の詳細情報 ==="
iw dev wlan1 info 2>&1 || echo "(wlan1 が見つかりません)"
echo ""
...
echo "=== 10. hostapd ログ（最新30行） ==="
sudo journalctl -u hostapd -n 30 --no-pager 2>&1 | tail -30
echo ""

echo "=== 11. dnsmasq ログ（最新30行） ==="
sudo journalctl -u dnsmasq -n 30 --no-pager 2>&1 | tail -30
echo ""

echo "=== 12. temperature-server ログ（最新30行） ==="
sudo journalctl -u temperature-server -n 30 --no-pager 2>&1 | tail -30
echo ""
